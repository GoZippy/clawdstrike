...
- [ ] Create `@clawdstrike/vercel-ai` with middleware + streaming support.
- [ ] Tool wrapping (pre/post) + model wrapping (bootstrap + prompt injection defenses).
- [ ] React helpers (`useSecureChat`, etc.) if they don’t materially expand scope.
- [ ] Examples + integration tests with mocked model/tools.

### B3. LangChain / LangGraph integration (P0)
- [ ] Create `@clawdstrike/langchain` using callbacks + tool wrappers.
- [ ] LangGraph nodes: “security checkpoint node”, tool-node wrapper, conditional edges.
- [ ] Ensure correct trace/correlation propagation for audit.
...
- [ ] Implement `policy.test.yaml` runner with fixtures, contexts, parameterization.
- [ ] Add coverage model (which guards/rules were exercised).
...
- [ ] Batch simulation mode (`events.jsonl` / audit replay) producing a report (counts, top denials).
- [ ] Policy A vs Policy B comparison mode.
...
- [ ] Add prompt-security audit events (including fingerprints/hashes for dedupe without storing raw prompts).

### D1. Jailbreak detection (tiered)
- [ ] Heuristics + normalization (fast).
- [ ] Statistical layer (entropy/n-grams/perplexity proxies).
- [ ] Optional ML classifier path (ONNX) with clear deployment requirements.
- [ ] Optional LLM-as-judge path with caching + cost controls.

### D2. Output sanitization
- [ ] Secret patterns + high-entropy detection improvements.
- [ ] PII detection (NER optional; provide “small/fast” and “large/accurate” modes).
- [ ] Streaming-safe sanitizer (don’t split tokens; bounded buffering).

### D3. Instruction hierarchy enforcement
- [ ] Standard “message tagging” and override detection strategy.
- [ ] Enforce system > developer > user > tool instruction priority in adapters.
- [ ] Audit “override attempts” (blocked/warned).

### D4. Watermarking + provenance (P1/P2 depending on scope)
- [ ] Define watermark payload + encoding (zero-width/homoglyph/metadata/hybrid).
