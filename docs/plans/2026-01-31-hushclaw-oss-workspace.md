# Hushclaw OSS Workspace Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create the hushclaw OSS repository with a complete Rust workspace containing crypto primitives (hush-core), network proxy utilities (hush-proxy), runtime guards (hushclaw), CLI tool (hush-cli), and daemon (hushd).

**Architecture:** Extract and adapt battle-tested code from Glia Fab's cyntra-trust (crypto) and aegis-daemon (guards/policy) crates into a clean OSS workspace. The workspace follows a layered design: hush-core (crypto primitives) -> hush-proxy (network utilities) -> hushclaw (guards + policy engine) -> hush-cli/hushd (user-facing tools).
...
All work happens in: `/Users/connor/Medica/hushclaw-ws1-oss-repo/`
...
[package]
name = "hush-core"
description = "Core cryptographic primitives for hushclaw"
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo && cargo test -p hush-core signing
```
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo && cargo test -p hush-core hashing
```
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo && cargo test -p hush-core canonical
```
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo && cargo test -p hush-core merkle
```
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo && cargo test -p hush-core receipt
```
...
[package]
name = "hush-proxy"
description = "Network proxy utilities for hushclaw"
version.workspace = true
edition.workspace = true
license.workspace = true
repository.workspace = true
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo && cargo test -p hush-proxy
```
...
## Task 10: hushclaw Crate - Core Structure
...
- Create: `crates/hushclaw/src/decision.rs`
...
```rust
//! Hushclaw - Runtime Security Guards for AI Agents
//!
...
impl Default for Policy {
    fn default() -> Self {
        Self {
            version: Some("hushclaw-v1.0".to_string()),
...
```bash
git add crates/hushclaw/
git commit -m "feat(hushclaw): add core types - error, policy, event, decision"
```

---

## Task 11: hushclaw Guards Module
...
```bash
mkdir -p crates/hushclaw/src/guards
git add crates/hushclaw/src/guards/mod.rs
git commit -m "feat(hushclaw): add Guard trait and GuardRegistry"
```
...
git commit -m "feat(hushclaw): add ForbiddenPathGuard"
```
...
(See source files in glia-fab for implementation details - adapt to use hushclaw types)
...
**Files:**
- Create: `crates/hushclaw/src/engine.rs`
...
```bash
git add crates/hushclaw/src/engine.rs
git commit -m "feat(hushclaw): add HushEngine evaluation core"
```
...
[package]
name = "hush-cli"
description = "Hushclaw command-line interface"
version.workspace = true
edition.workspace = true
license.workspace = true
repository.workspace = true
...
#[derive(Parser)]
#[command(name = "hush")]
#[command(about = "Hushclaw security policy enforcement CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Validate { policy } => {
            let policy = hushclaw::Policy::from_yaml_file(&policy)?;
...
[package]
name = "hushd"
description = "Hushclaw daemon for runtime enforcement"
version.workspace = true
edition.workspace = true
license.workspace = true
repository.workspace = true
...
#[derive(Parser)]
#[command(name = "hushd")]
#[command(about = "Hushclaw security enforcement daemon")]
struct Args {
    /// Path to policy file
    #[arg(short, long, default_value = "/etc/hushclaw/policy.yaml")]
    policy: PathBuf,
...
async fn main() -> anyhow::Result<()> {
    ...
    let policy = if args.policy.exists() {
        hushclaw::Policy::from_yaml_file(&args.policy)?
    } else {
        tracing::warn!("Policy file not found, using defaults");
        hushclaw::Policy::default()
    };

    let engine = hushclaw::HushEngine::new(policy);
...
Hushclaw provides a modular security framework for AI agent runtimes:
...
# Run daemon
hushd --policy /etc/hushclaw/policy.yaml
```
...
| `hushclaw` | Runtime guards and policy engine |
...
```bash
cd /Users/connor/Medica/hushclaw-ws1-oss-repo
cargo build --all
```
...
This plan creates a complete hushclaw OSS workspace with:
...
3. **hushclaw**: Guards + policy engine (5 guards, HushEngine)
